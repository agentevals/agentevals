...
assert: Evaluator[]       # Optional: Default evaluators
...
execution:
  ...
  # DEPRECATED: evaluators has moved to the top-level `assert` key
...
tests:
  - id: string                # Required: Unique identifier
    ...
    assert: Evaluator[]       # Per-test evaluators
...
```yaml
type: skill-trigger
skill: string       # Required: Skill name (case-sensitive substring match)
should_trigger: boolean  # Optional: Expected trigger outcome (default: true)
```
